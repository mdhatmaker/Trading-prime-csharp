...
{
    public class WebsocketHuobi
    {
        private static readonly string WssUrl = "wss://api.huobi.pro/ws";

        // supported pairs: BCCBTC, LTCBTC, ETHBTC, ETCBTC
        ...
        public WebsocketHuobi(ApiCredentials credentials)
        {
            Console.WriteLine("WebsocketHuobi::ctor()  {0} {1}", credentials.ApiKey, credentials.ApiSecret);
        ...
        public bool Connect()
        {
            Console.WriteLine("WebsocketHuobi::Connect()");
        ...
        {
            Console.WriteLine("WebsocketHuobi::Socket_OnOpened()");
            ...
            Console.WriteLine("WebsocketHuobi->emitting auth credentials...");
            socket.Emit("auth", this.credentials);
        }

        void Socket_OnData(byte[] data)
        {
            //string zipped = System.Text.Encoding.UTF8.GetString(data);
            string zipped = System.Text.Encoding.Unicode.GetString(data);
            string unzipped = Util.UnZip(zipped);
            Console.WriteLine("WebsocketCoinigy::Socket_OnData()  {0}", unzipped);
        ...
        {
            Console.WriteLine("WebsocketHuobi::Socket_OnMessage()  {0}", message);
        ...
        {
            Console.WriteLine("WebsocketHuobi::GetRequestType()  {0}", jObj);
        ...
        {
            Console.WriteLine("WebsocketHuobi::InvokeMessageReceived()  {0} {1} {2}", channelName, requestType, message);
        ...
        {
            Console.WriteLine("WebsocketHuobi::GetMessageType()  {0}", requestType);
    ...
    } // end of class WebsocketHuobi




    public static class TestWebsocketHuobi
    {
        private static readonly ManualResetEvent resetEvent = new ManualResetEvent(false);
        private static WebsocketHuobi socket;

        public static void TryMe()
        {
            Console.WriteLine("TestWebsocketHuobi::TryMe()");

            // Initialize an instance of our socket
            socket = new WebsocketHuobi(new ApiCredentials
            {
                ApiKey = "314b227a-5655127f-7eb0aded-04938",
                ApiSecret = "9e7a3a46-6430a7b8-ed1bc72a-838e6"
            });
        ...
        {
            Console.WriteLine("TestWebsocketHuobi::Socket_OnClientReady()");
            // Subscribe to a new trade channel
            socket.SubscribeToTradeChannel("BMEX", "XBT", "USD");
        }
    } // end of class TestWebsocketHuobi
