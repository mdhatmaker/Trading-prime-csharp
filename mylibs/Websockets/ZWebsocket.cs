...
{
    public class ZWebsocket : IBasicListener
    {
        ...
        {
            string channelId = $"TRADE-{exchange}--{primaryCurrency}--{secondaryCurrency}";
            Console.WriteLine("subscribing to: {0}", channelId);

            return this.socket.Subscribe(channelId);

            /*var channel = socket.GetChannelByName(channelId).Subscribe();
            channel.OnMessage((name, data) =>
            {
                Console.WriteLine("Got message for channel:: " + name + " data :: " + data);
            });
            //socket.OnSubscribe()
            return true;*/
        }
        ...
        public void OnSetAuthToken(string token, Socket socket)
        {
            ...
            socket.On("#publish", (name, data, ack) =>
            {
                Console.WriteLine("got message " + data + " from event " + name);
                //ack(name, "No error", "Hi there buddy");
            });

            socket.On("#setAuthToken", (name, data, ack) =>
            {
                Console.WriteLine("got message " + data + " from event " + name);
            });

            socket.Emit("exchanges", null);
            socket.Emit("channels", "OK");
        }

    } // end of class ZWebsocket
