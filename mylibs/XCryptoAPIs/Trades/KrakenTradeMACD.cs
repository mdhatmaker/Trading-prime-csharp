...
namespace CryptoAPIs
{
    public class KrakenTradeMACD
    {
        ...
        private static string m_tradeFile = "KRAKEN.MACD.trades.{0}.{1}.DF.csv";   // format this string with pair ("BTCEUR", etc.) and datetime ("yyyyMMddHHmmss") to get trade filename

        public KrakenTradeMACD(Kraken exch, string pair)
        {
            m_exch = exch;
            m_pair = pair;
        }
        ...
        void GetOHLC(int barIntervalMinutes, bool displayIndicator, bool displayDebug = false)
        {
            ...
            var ohlc = m_exch.GetOHLC(m_pair, barIntervalMinutes);
        ...
        OrderNew SubmitOrder(OrderSide side, decimal price, decimal size, int leverage=2)
        {
            if (m_tradeLive == false) return null;       // only submit trades when m_tradeLive is true

            m_exch.SetLeverage(leverage);
        ...
        void TradeBuy()
        {
            var ticker = m_exch.GetKrakenTicker(m_pair);
            cout("\n\n{0}>---BUY:", m_pair);
            ticker.Display("ticker:");
            var t = ticker.Values.First();
            string msg = string.Format("{0}  buy {1} unit(s) pay {2} (asksize:{3})    [LONG TRADE: {4}]\n", m_pair, m_tradeUnits, t.AskPrice, t.AskVolume, DateTime.Now.ToShortTimeString());
            cout(msg);
            decimal tradePrice = t.AskPrice;                        // assume ASK price for BUY
            decimal tradeSize = m_tradeUnits * m_tradeUnitSize;     // trade size is trade_units * unit_size
            SubmitOrder(OrderSide.Buy, tradePrice, tradeSize, 2);
            msg += GetTradeBalances();
            TelegramBot.Instance.SendToAll(msg);
            m_pos += 2;
            LogTrade(string.Format("{0},{1},{2},{3}u,{4},{5},{6}x{7}:{8}x{9}", DateTime.Now.ToSortableDateTime(), m_pair, "buy", m_tradeUnits, tradeSize, tradePrice, t.BidVolume, t.BidPrice, t.AskPrice, t.AskVolume));
        ...
        void TradeSell()
        {
            ...
            ticker.Display("ticker:");
            var t = ticker.Values.First();
            var msg = string.Format("{0}  sell {1} unit(s) at {2} (bidsize:{3})    [SHORT TRADE: {4}]\n", m_pair, m_tradeUnits, t.BidPrice, t.BidVolume, DateTime.Now.ToShortTimeString());
            decimal tradePrice = t.BidPrice;                        // assume BID price for SELL
            decimal tradeSize = m_tradeUnits * m_tradeUnitSize;     // trade size is trade_units * unit_size
            SubmitOrder(OrderSide.Sell, tradePrice, tradeSize, 2);
            msg += GetTradeBalances();
            TelegramBot.Instance.SendToAll(msg);
            m_pos -= 2;
            LogTrade(string.Format("{0},{1},{2},{3}u,{4},{5},{6}x{7}:{8}x{9}", DateTime.Now.ToSortableDateTime(), m_pair, "sell", m_tradeUnits, m_tradeUnits * m_tradeUnitSize, tradePrice, t.BidVolume, t.BidPrice, t.AskPrice, t.AskVolume));
            m_tradeUnits = 2;                       // after the first trade, we always buy/sell 2 units
        }
