    ...
    public class BitfinexApiV1
    {
        ...
        private string SendRequest(GenericRequest request, string httpMethod)
        {
            ...
            catch (WebException ex)
            {
                ...
                response = sr.ReadToEnd();
                sr.Close();
                //throw new BitfinexException(ex, response);  // TODO: REALLY need to handle (and prevent) the "TOO MANY REQUESTS" error (90 per second max?)                
                ErrorMessage("BitfinexClient::SendRequest=> {0} ... {1}", ex.Message, response);
                return null;
            }
            return response;
        }
    } // end of class BitfinexApiV1
